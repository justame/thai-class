...
describe('needsTranslation', () => {
  it('should be true when english is empty', () => {
    expect(needsTranslation({ thai: 'ก', english: '' } as Word)).toBe(true);
  });
  ...
  it('should be false when english is present', () => {
    expect(needsTranslation({ thai: 'ก', english: 'a' } as Word)).toBe(false);
  });
});

describe('addTranslations', () => {
  it('should skip the API call when every word already has english', async () => {
    const words = [{ thai: 'ก', english: 'a' }] as Word[];
