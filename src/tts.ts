...
const PROVIDERS: Record<string, Provider> = { google, elevenlabs };
...
async function synthesizeChunk(chunk: Chunk, state: TtsState): Promise<SynthesizedAudio> {
  const primaryKey = TTS_PROVIDER in PROVIDERS ? TTS_PROVIDER : 'google';
  if (primaryKey === 'google' || state.primaryDead) return google.synthesizeChunk(chunk);
  try {
    return await PROVIDERS[primaryKey].synthesizeChunk(chunk);
  } catch (err) {
    state.primaryDead = true;
    console.warn(`WARN: ${TTS_PROVIDER} TTS failed (${(err as Error).message}); using Google for the rest of this lesson`);
    return google.synthesizeChunk(chunk);
  }
}

// LLM-provided pause, clamped so a bad value can't make a giant gap or none at all.
function pauseFor(chunk: Chunk): number {
...
function getDurationSeconds(audioPath: string): Promise<number | null> {
...
async function stitchToMp3(segmentPaths: string[], pauses: number[], mp3Path: string): Promise<void> {
...
export async function makeAudio(chunks: Chunk[], { outPath }: MakeAudioOptions = {}): Promise<AudioResult> {
