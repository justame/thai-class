...
interface KeepResult {
  results: { thai: string; keep: boolean }[];
}

interface ObviousResult {
  results: { thai: string; obvious: boolean }[];
}
...
export interface KeepContentWordsOptions {
  apiKey?: string;
...
export async function keepContentWords(
  words: string[],
  { apiKey, onProgress }: KeepContentWordsOptions = {},
): Promise<string[]> {
  ...
  for (let i = 0; i < words.length; i += BATCH_SIZE) {
    ...
    const { results } = JSON.parse(getMessageContent(completion)) as KeepResult;
...
function obviousPrompt(batch: string[]): string {
...
export function needsObviousTag(word: Word): boolean {
  return typeof word.obvious !== 'boolean';
}

export interface TagObviousWordsOptions {
  apiKey?: string;
...
// (see srs.ts getNewWords). Already-tagged words are returned as-is.
export async function tagObviousWords(
  words: Word[],
  { apiKey, onProgress }: TagObviousWordsOptions = {},
): Promise<Word[]> {
  const missing = words.filter(needsObviousTag);
  if (missing.length === 0) return words;

  const client = new OpenAI({ apiKey: apiKey ?? process.env.OPENAI_API_KEY });
  const byThai = new Map<string, boolean>();
  ...
  for (let i = 0; i < missing.length; i += BATCH_SIZE) {
    ...
    const { results } = JSON.parse(getMessageContent(completion)) as ObviousResult;
