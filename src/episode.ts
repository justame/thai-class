...
export function buildTitle(words: Word[], episodeNumber: number): string {
  const thaiList = words.map((w) => w.thai).join(', ');
  return `Daily Thai #${episodeNumber} — ${thaiList}`;
}

export function buildDescription(reviews: Word[], news: Word[]): string {
  const lines = ['Today:', ''];
  if (news.length) {
...
export function getEpisodeNumber(arg: string | undefined, lastEpisodeNumber?: number): number {
