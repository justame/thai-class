...
export interface MixedScriptProblem {
  index: number;
  speaker: string;
  lang: string;
  text: string;
...
export function findMixedScriptLines(chunks: Chunk[]): MixedScriptProblem[] {
  const problems: MixedScriptProblem[] = [];
...
export function checkNoMixedScript(chunks: Chunk[]): Chunk[] {
