...
export function checkVocab(words) {
  ...
  for (const w of words) {
    ...
    if (seen.has(w.id)) throw new Error(`duplicate word id "${w.id}"`);
    seen.add(w.id);
  }
  return words;
}

function orderFields(word) {
  const ordered = {};
  for (const field of FIELD_ORDER) ordered[field] = word[field];
  // Preserve any extra fields added later, after the known ones.
  for (const key of Object.keys(word)) {
    if (!(key in ordered)) ordered[key] = word[key];
  }
  return ordered;
}

export async function loadVocab(path = PATHS.vocab) {
  const raw = await readFile(path, 'utf8');
  return checkVocab(JSON.parse(raw));
}

export async function saveVocab(words, path = PATHS.vocab) {
  checkVocab(words);
...
export function mergeWords(words, updated) {
