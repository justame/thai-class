---
name: Beginner Thai Audio Class
description: A beginner-first 3-person Thai class. Understandable before immersive — English anchors meaning, one word + one pattern per episode, the listener practices along.
---
# Beginner Thai Audio Class

Write a beginner Thai audio lesson that feels like a simple, clear classroom scene the
listener can follow every step of. The listener is a TRUE BEGINNER: they do not understand
full Thai sentences yet. Make it understandable FIRST, immersive second.

## Beginner rules (most important)
...
Use natural everyday Thai. If a form is formal, say so in English; if casual, say so.

## Teach a pattern, not just the dictionary meaning

Give one useful beginner pattern for the word, e.g. for ไป: ไปไหน (where are you going),
ไปที่ (go to), ไปกับ (go with), จะไป (going to go). Pick ONE pattern for the episode.
Only show a correction if there is a GENUINE, common beginner mistake for this word/pattern.
If there isn't a natural mistake, skip the mistake step and just teach through clear examples.
...
- "new_word" — right before the main Thai word is introduced.
- "try" — right before student1 (or the listener) attempts something.
- "correct" — right after a correct answer, as a happy little reward.
- "practice" — right before the listener-practice line.
- "recap" — right before the final recap.
Use 5-7 cues total across the episode. "start", "new_word", "practice", "recap" should
each appear once; "try" and "correct" can appear a couple of times at natural moments.
...
NEW word to teach today (use the first one as the single main word):
{{NEW_WORDS}}

REVIEW word to weave in if natural:
{{REVIEW_WORDS}}

level: true beginner — take the time the lesson needs (roughly 1.5-3 minutes). Quality
